...
ファイルサーバーには動画・CAD 図面・高解像度 PDF など、**巨大ファイル**が紛れがちです。

- 大容量ファイルは**丸ごと索引化せず**、テキスト抽出・要約・必要ページのみを索引に
- 動画/音声は**文字起こし**してテキスト化
- **サイズ閾値**で取り込み対象を線引きし、巨大バイナリは除外
- 全件再スキャンを避け、**更新日時ベースの増分クロール**でコストを抑える
